...
async def lifespan(app: APIRouter):
    ...
    print(
        "=== FASTAPI STARTUP DIAGNOSTICS ==="
        )
    print(
        f"Total routes registered: {len(app.routes)}"
        )
    for route in app.routes:
        if hasattr(
            ...
            print(
                f"Route: {methods_list} {route.path}"
                )
    print(
        "=== END DIAGNOSTICS ==="
        )
...
async def render_todo_page(request: Request, db: db_dependency):
    try:
        ...
        todos = db.query(Todos).filter(Todos.owner_id == user.get(
            "id")).all()
...
async def read_todo(user: user_dependency,
    ...
    if user is None:
        raise HTTPException(status_code=401, detail='Authentication failed.')
    todo_model = db.query(Todos).filter(Todos.id ==
                                        todo_id).filter(
